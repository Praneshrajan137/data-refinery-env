...
@pytest.mark.integration
def test_verify_fixes_curated_batch_yields_guardrail_split(client: TestClient) -> None:
    """The scripted untrusted-agent batch proves exactly one edit and blocks three, each with a
    distinct honest reason.

    **This assertion was absent between 2026-09-07 and 2026-09-08, and that is why the demo
    silently rotted.** C4 stopped a MINED constraint accepted in review from conferring write
    authority, which was the only premise this scenario had, so the split collapsed to uniform
    refusal. The test was then rewritten to assert `would_apply == []` -- i.e. to assert the
    degradation -- which left it indistinguishable from
    `test_verify_fixes_without_schema_holds_everything` and unable to notice the loss.

    The split is restored by a **DECLARED** premise (`hospital_10rows.schema.yaml`, generated by
    `scripts/playground/build_samples.py`), not by a flag. `mined_constraints_grant_write_authority`
    is still deliberately NOT used: the playground must never show a write the shipped CLI default
    refuses.

    Asserting a non-empty `would_apply` is the point. A guardrail that can only ever refuse
    demonstrates nothing, and `docs/trust/fd-repair-yield-mechanism.md` establishes that a declared
    premise produces genuinely proven writes at precision 1.0000 on a real corpus -- so this is not
    a demo of a capability that fails to generalise.
    """
    name = "hospital_10rows"
    scenario = client.get(f"/api/verify-scenarios/{name}").json()
    csv_bytes = client.get(f"/api/samples/{name}").content

    assert scenario["declared_schema"] == name, "hospital ships a declared premise"
    ...
    response = client.post(
        ...
        data={
            ...
            "declared_schema": scenario["declared_schema"],
        },
    )
    assert response.status_code == 200
    body = response.json()
    ...
    # THE assertion whose absence let this rot: something must actually be proven.
    assert len(body["would_apply"]) == 1, (
        "the correctly-typed edit must be proven against the declared premise; a guardrail that "
        "only ever refuses demonstrates nothing"
    )
    proven = body["would_apply"][0]
    assert (proven["row"], proven["column"]) == (0, "er_wait_time")

    # And the three genuinely bad proposals are each blocked for their own distinct reason.
    ...
    # With a real write candidate present, the differential SMT+Direct check runs and agrees.
    # This was honestly "not_run" while nothing could be proven; asserting "agreed" is only
    # legitimate now that a candidate actually reaches that gate.
    assert body["receipt"]["independent_verification"] == "agreed"


@pytest.mark.integration
@pytest.mark.parametrize("name", ["flights_10rows", "beers_10rows"])
def test_verify_fixes_without_a_declared_premise_proves_nothing(
    client: TestClient, name: str
) -> None:
    """A sample with no declared premise refuses everything, and says so honestly.

    The contrast with the hospital scenario above is the product's actual claim: accepting a
    constraint MINED from the dirty table is not evidence enough to authorise a write (C4), so
    these batches prove nothing even though their fixes are shaped identically. Keeping both
    behaviours under test is what stops either from drifting unnoticed.

    `authoritative_schema` must be False here. Before 2026-09-08 it was computed from
    `bool(accepted_constraint_ids)` and so reported True while nothing could be proven -- a
    surface claiming an authoritative schema it did not have.
    """
    scenario = client.get(f"/api/verify-scenarios/{name}").json()
    csv_bytes = client.get(f"/api/samples/{name}").content

    assert scenario["declared_schema"] is None, f"{name} ships no declared premise"
...
@pytest.mark.integration
def test_the_bundled_sample_schema_matches_its_generator(client: TestClient) -> None:
    """The declared premise on disk must equal what `build_samples.py` emits.

    The schema is generated, and the sample CSV is generated, so nothing stops them drifting
    apart by hand. This pins the artifact to its generator and the generator to the CSV header --
    a premise naming a column the sample does not have would silently prove nothing.
    """
    import importlib.util

    root = Path(__file__).resolve().parents[2]
    spec = importlib.util.spec_from_file_location(
        "_build_samples", root / "scripts" / "playground" / "build_samples.py"
    )
    assert spec is not None and spec.loader is not None
    generator = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(generator)

    on_disk = (root / "playground" / "api" / "samples" / "hospital_10rows.schema.yaml").read_text(
        encoding="utf-8"
    )
    assert on_disk.replace("\r\n", "\n") == generator.HOSPITAL_STORY_SCHEMA_YAML.replace(
        "\r\n", "\n"
    ), "hospital_10rows.schema.yaml has drifted from build_samples.py; regenerate it"

    csv_header = (
        (root / "playground" / "api" / "samples" / "hospital_10rows.csv")
    ...
    declared = schema_from_mapping(yaml.safe_load(generator.HOSPITAL_STORY_SCHEMA_YAML))
    assert set(declared.columns) == set(csv_header), (
        "the declared premise and the sample CSV must describe the same columns"
    )
    # The discriminating types are what confer authority; `str` columns are inert by design.
    from dataforge.engine.repair import authoritative_columns

    assert authoritative_columns(declared) == {
        "rating",
        "mortality_rate",
        "readmission_rate",
        "er_wait_time",
