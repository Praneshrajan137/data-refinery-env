...
def _write_sdist(path: Path, members: set[str]) -> None:
    ...
    with tarfile.open(path, "w:gz") as archive:
        for member in sorted(members):
            payload = b""
            info = tarfile.TarInfo(f"dataforge-0.1.0/{member}")
...
def test_wheel_contents_audit_accepts_allowed_surface(tmp_path: Path) -> None:
    wheel_path = tmp_path / "dataforge-0.1.0-py3-none-any.whl"
    members = set(REQUIRED_WHEEL_MEMBERS) | {
        "dataforge/__init__.py",
        "dataforge-0.1.0.dist-info/METADATA",
        "dataforge-0.1.0.dist-info/WHEEL",
        "dataforge-0.1.0.dist-info/RECORD",
    }
...
def test_sdist_contents_audit_accepts_allowed_surface(tmp_path: Path) -> None:
    sdist_path = tmp_path / "dataforge-0.1.0.tar.gz"
    members = set(REQUIRED_SDIST_MEMBERS) | {
        "dataforge.egg-info/PKG-INFO",
        "dataforge.egg-info/SOURCES.txt",
        "dataforge.egg-info/requires.txt",
        "dataforge.egg-info/entry_points.txt",
        "dataforge.egg-info/top_level.txt",
        "dataforge.egg-info/dependency_links.txt",
    }
