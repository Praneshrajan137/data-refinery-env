...
def test_kaggle_grpo_candidate_bundle_uses_configured_v7_curriculum_and_predecessor(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(prepare_kaggle_grpo_candidate, "SOURCE_ROOTS", ("pyproject.toml",))
    ...
    assert candidate_manifest["trajectory_file"] == "expert_v7_parse_latch.jsonl"
    assert candidate_manifest["sft_predecessor_report_file"] == "sft_v7_candidate_eval_report.json"


def test_kaggle_grpo_candidate_bundle_uses_v9_predecessor_for_v4(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(prepare_kaggle_grpo_candidate, "SOURCE_ROOTS", ("pyproject.toml",))
    ...
    assert candidate_manifest["trajectory_file"] == "expert_v9_action_envelope.jsonl"
    assert candidate_manifest["sft_predecessor_report_file"] == "sft_v9_candidate_eval_report.json"
