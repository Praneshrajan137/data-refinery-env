...
| Deterministic correction accuracy | **Narrow, and configuration-dependent** | hospital F1 0.8352 at **proposal stage** (one dataset with injected errors, under our scoring); end-to-end through the shipped write path under a declared premise: **0.0000 by default** (a >100-cell batch is discarded silently) and **0.4599 at precision 1.0000 with zero corruptions** with `--confirm-escalations` ([fd-repair-yield-mechanism](trust/fd-repair-yield-mechanism.md)); flights 0.00 deterministic -> **0.4467 with cross-row entity consensus** (`allow_entity_consensus`); tax sampled 0.00 with 708 false positives |
