...
try:
    ...
    # Fallback for direct execution (``python server/data_quality_environment.py``)
    from compat import Environment  # type: ignore[no-redef]
...
class DataQualityEnvironment(Environment):
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        if task_id not in VALID_TASK_IDS:
            logger.warning(
                "Unknown task_id %r — defaulting to 'task_1_format_fixer'. "
                "Valid: %s",
                task_id,
    ...
    def step(self, action: DataQualityAction) -> DataQualityObservation:
        ...
        if done and not self._is_finalized:
            ...
            message += (
                f" Max steps reached — auto-finalized. "
                f"Final score: {self.cumulative_reward:.4f}"
            )
            logger.info(
    ...
    def _handle_inspect(
        ...
        if action.row_indices is not None:
            indices = [
                i for i in action.row_indices[:10]
                if 0 <= i < len(self.dataset)
            ]
            if indices:
                visible_rows = [
                    {**self.dataset[i], "_row_index": i} for i in indices
                ]
        ...
        if action.column_names is not None:
            ...
            for col in action.column_names:
                ...
                if numeric_vals:
                    ...
                    stats["mean"] = round(
                        sum(numeric_vals) / len(numeric_vals), 4
                    )
    ...
    def _handle_diagnose(
        ...
        for truth in self.ground_truth:
            ...
            col_match = (
                truth["column"] == action.column_name
                or truth["column"] == "_row"
            )
    ...
    def _handle_fix(
        ...
        is_delete_action = (
            action.fix_type is not None
            and (
                action.fix_type == FixType.DELETE_ROW
                or (
                    isinstance(action.fix_type, str)
                    and action.fix_type == "delete_row"
                )
            )
        )
            ...
            if row_match and col_match:
                ...
                if provided.lower() == str(expected).lower():
                    ...
                    return (
                        P_WRONG_FIX,
                        ActionResult.INCORRECT,
                        f"Wrong fix value at row {action.row_index}. "
                        f"Penalty: {P_WRONG_FIX}",
                    )
        ...
        return (
            P_WRONG_FIX,
            ActionResult.INCORRECT,
            f"No known issue at row {action.row_index}, "
            f"column '{action.column_name}'.",
        )
    ...
    def _handle_finalize(self) -> DataQualityObservation:
        ...
        logger.info(
            "Episode %s finalized: score=%.4f, detected=%d/%d, "
            "fixed=%d, false_pos=%d",
            self._state.episode_id[:8],
            final_score,
...
if __name__ == "__main__":
    ...
    def _test_duplicate_any_column() -> None:
        ...
        action = DataQualityAction.diagnose(
            row_index=12, column_name="email",
    ...
    def _test_oob_diagnose() -> None:
        env.reset(task_id="task_1_format_fixer")
        action = DataQualityAction.diagnose(
            row_index=9999, column_name="email",
            issue_type=IssueType.FORMAT_ERROR,
        )
        obs = env.step(action)
