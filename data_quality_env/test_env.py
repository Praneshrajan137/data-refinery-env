...
def _load_ground_truth(task_id: str) -> List[Dict[str, Any]]:
    ...
    gt_path = Path(__file__).resolve().parents[1] / "datasets" / config["ground_truth"]
...
def test_reset_all_tasks() -> None:
    ...
    for task_id in TASK_CONFIG:
        ...
        _check(
            f"reset({task_id}) returns DataQualityObservation",
        ...
        _check(
            f"  visible_rows populated", obs.visible_rows is not None and len(obs.visible_rows) > 0
        )
        ...
        _check(
            f"  issues_remaining_hint is enum", isinstance(obs.issues_remaining_hint, RemainingHint)
        )
...
def test_inspect_rows() -> None:
    ...
    _check("_row_index values correct", [r["_row_index"] for r in obs.visible_rows] == [0, 1, 2])
...
def test_inspect_out_of_bounds_rows() -> None:
    ...
    _check(
        "OOB indices filtered, valid returned",
...
def test_diagnose_correct_hardcoded() -> None:
    ...
    _check(
        f"reward_delta={expected_reward:.2f} (R_DIAGNOSE + R_TYPE_BONUS)",
...
def test_diagnose_correct_dynamic() -> None:
    ...
    for task_id in TASK_CONFIG:
        ...
        _check(
            f"{task_id}: row {issue['row']}, {issue['column']} -> correct",
...
def test_diagnose_false_positive() -> None:
    ...
    _check(
        f"reward_delta clamped (P_FALSE_POS applied internally)",
...
def test_diagnose_duplicate_any_column() -> None:
    ...
    if dup_issue is None:
        _check(
            "SKIP: no _row issue in task 2 ground truth",
            False,
    ...
    _check(
        "hardcoded: row 12 is duplicate",
        dup_issue["row"] == 12 or any(g["row"] == 12 and g["column"] == "_row" for g in gt),
    )
    ...
    for col in test_columns:
        ...
        _check(
            f"dup row {dup_issue['row']} via '{col}' -> correct",
            obs.action_result == ActionResult.CORRECT,
            f"got {obs.action_result}",
        )
...
def test_fix_correct_hardcoded() -> None:
    ...
    _check(
        f"reward_delta={expected_reward:.2f} (R_FIX + R_JUSTIFY_BONUS)",
        _approx(obs.reward_delta, expected_reward),
        f"got {obs.reward_delta}",
    )
...
def test_fix_correct_dynamic() -> None:
    ...
    for task_id in TASK_CONFIG:
        ...
        _check(
            f"{task_id}: fix row {issue['row']}/{col} -> correct",
            obs.action_result == ActionResult.CORRECT,
            f"got {obs.action_result}",
        )
...
def test_fix_detection_only() -> None:
    ...
    if det_only is None:
        _check(
            "SKIP: no detection-only issue in task 1",
            False,
    ...
    _check(
        "hardcoded: row 7 phone is detection-only",
    ...
    _check(
        "detection-only fix returns partial",
...
def test_fix_delete_row() -> None:
    ...
    if dup_issue is None:
        _check(
            "SKIP: no DELETE_ROW issue in task 2", False, "Expected at least one DELETE_ROW issue"
        )
        return

    # Hardcoded verification
    _check(
        "hardcoded: row 12 has DELETE_ROW expected",
    ...
    _check(
        "DELETE_ROW fix returns correct",
        obs.action_result == ActionResult.CORRECT,
        f"got {obs.action_result}",
    )
...
def test_finalize_no_work() -> None:
    ...
    _check("score ~0 (no work)", obs.cumulative_reward < 0.1, f"got {obs.cumulative_reward}")
...
def test_cumulative_reward_accumulation() -> None:
    ...
    for issue in gt:
        ...
        if obs.action_result == ActionResult.CORRECT:
            ...
            _check(
                f"step {diagnosed + 1}: cumulative matches",
...
def test_final_score_formula() -> None:
    ...
    _check(
        f"all-diagnose score ~{expected_score:.4f} ({DETECTION_WEIGHT}*1.0 + {FIX_WEIGHT}*0.0)",
...
def test_full_episode_perfect_score() -> None:
    ...
    _check(
        f"perfect score ~{expected_perfect:.4f}",
...
def test_pydantic_validation_boundaries() -> None:
    ...
    invalid_cases = [
        ("inspect with no fields", lambda: DataQualityAction(action_type="inspect")),
        (
        ...
        (
            ...
            lambda: DataQualityAction(
                action_type="diagnose", row_index=0, column_name="a", issue_type="outlire"
            ),
        ),
    ]
...
def test_env_level_invalid_action() -> None:
    ...
    try:
        ...
        _check(
            "nonexistent column diagnosed as false positive",
            obs.action_result == ActionResult.INCORRECT,
        )
    except Exception:
...
def test_all_tasks_ground_truth_integrity() -> None:
    ...
    for task_id in TASK_CONFIG:
        ...
        _check(
            f"{task_id}: has >= {min_expected} issues", len(gt) >= min_expected, f"has {len(gt)}"
        )
        ...
        fixable = [g for g in gt if g.get("expected") is not None]
        _check(f"{task_id}: has fixable issues", len(fixable) >= 1, f"has {len(fixable)}")

        # At least some detection-only
        detection_only = [g for g in gt if "expected" not in g]
        _check(
            f"{task_id}: has detection-only issues",
            len(detection_only) >= 1,
...
def test_state_property() -> None:
    ...
    from .server.data_quality_environment import DataQualityEnvironment as _Env
...
def test_remaining_hint_progression() -> None:
    ...
    _check(
        "hint progression includes multiple values", len(hints_seen) >= 2, f"only saw: {hints_seen}"
    )
...
def test_perfect_score_is_1_0() -> None:
    ...
    obs = env.step(DataQualityAction(action_type="finalize"))
    _check(
        "perfect score == 0.9999 (strict open interval)",
        _approx(obs.cumulative_reward, 0.9999),
        f"got {obs.cumulative_reward}",
    )
    _check(
        "perfect score strictly < 1.0", obs.cumulative_reward < 1.0, f"got {obs.cumulative_reward}"
    )
    _check("perfect score > 0.99", obs.cumulative_reward > 0.99, f"got {obs.cumulative_reward}")
...
def test_zero_score_is_0_0() -> None:
    ...
    obs = env.step(DataQualityAction(action_type="finalize"))
    _check(
        "zero score == 0.0001 (strict open interval)",
        _approx(obs.cumulative_reward, 0.0001),
        f"got {obs.cumulative_reward}",
    )
    _check("zero score strictly > 0.0", obs.cumulative_reward > 0.0, f"got {obs.cumulative_reward}")
    _check("zero score < 0.01", obs.cumulative_reward < 0.01, f"got {obs.cumulative_reward}")
...
def test_numeric_partial_counts_in_fix_rate() -> None:
    ...
    _check(
        "fix_rate > 0 in final score (numeric partial counted)",
...
def test_reinspect_penalty() -> None:
    ...
    _check("first inspect reward >= 0", obs1.reward_delta >= 0, f"got {obs1.reward_delta}")
    ...
    _check(
        "re-inspect penalty: reward_delta clamped",
...
def test_spam_penalty_uncapped() -> None:
    ...
    _check(
        "spam penalty drives score to 0",
...
def test_task3_null_total_fixable() -> None:
    ...
    gt42 = next(g for g in env.ground_truth if g["row"] == 42 and g["column"] == "order_total")
...
def test_task3_cascading_qty_mismatch() -> None:
    ...
    gt65 = next(g for g in env.ground_truth if g["row"] == 65 and g["column"] == "quantity")
    ...
    expected_total = round(
        expected_qty * row65["unit_price"] * (1 - row65["discount_pct"] / 100), 2
    )
    _check("row 65 total matches original qty", abs(row65["order_total"] - expected_total) < 0.015)
...
def test_task3_hidden_business_rules() -> None:
    ...
    _check("min_unit_price in business_rules", "min_unit_price" in env.business_rules)
    _check("min_discount_pct in business_rules", "min_discount_pct" in env.business_rules)
    ...
    # Fix row 115
    gt115 = next(g for g in env.ground_truth if g["row"] == 115 and g["column"] == "unit_price")
    obs = env.step(
        DataQualityAction(
            action_type="fix",
            row_index=115,
...
def test_task3_temporal_consistency() -> None:
    ...
    _check("row 160 ship_date < order_date", row160["ship_date"] < row160["order_date"])

    # Row 225: ship_date way in future
    _check("row 225 ship_date is 2029", env.dataset[225]["ship_date"].startswith("2029"))
    ...
    _check("temporal diagnose accepted", obs.action_result == ActionResult.CORRECT)
...
def test_coverage_exploration_bonus() -> None:
    ...
    _check(
        "first bonus >= second bonus (coverage decay)",
        bonus1 >= bonus2,
...
def test_inference_end_line_includes_explicit_score() -> None:
    ...
    _check(
        "reward trace preserves four decimals", line.endswith("rewards=0.1500,0.0001,0.3333"), line
    )
...
def test_inference_run_task_stdout_contract() -> None:
    ...
    _check(
        "normal case emits [START]",
        any(line.startswith("[START] ") for line in normal_lines),
        str(normal_lines),
    )
    _check(
        "normal case emits [STEP]",
        any(line.startswith("[STEP] ") for line in normal_lines),
        str(normal_lines),
    )
    _check(
        "normal case emits [END]",
        any(line.startswith("[END] ") for line in normal_lines),
        str(normal_lines),
    )
    _check(
        "normal [END] includes explicit score",
        any(" score=0.4321 " in line for line in normal_lines),
        str(normal_lines),
    )
    _check(
        "normal [STEP] uses validator field order",
        any(
            line == "[STEP] step=1 action=finalize() reward=0.4321 done=true error=null"
    ...
    _check(
        "normal parsed score in (0,1)",
        normal_end_score is not None and 0.0 < normal_end_score < 1.0,
        str(normal_lines),
    )
    ...
    _check(
        "fallback [END] keeps score above zero",
        any(" score=0.0001 " in line for line in fallback_lines),
        str(fallback_lines),
    )
    _check(
        "fallback parsed score in (0,1)",
        fallback_end_score is not None and 0.0 < fallback_end_score < 1.0,
        str(fallback_lines),
    )
    _check(
        "fallback run_task returns clamped score",
        _approx(fallback_score, 0.0001),
        str(fallback_score),
    )
    ...
    _check(
        "exception case still emits [END]",
        any(line.startswith("[END] ") for line in exception_lines),
        str(exception_lines),
    )
    _check(
        "exception case emits safe score",
        any(" score=0.0001 " in line for line in exception_lines),
        str(exception_lines),
    )
    _check(
        "exception parsed score in (0,1)",
        exception_end_score is not None and 0.0 < exception_end_score < 1.0,
        str(exception_lines),
    )
    _check(
        "exception run_task returns clamped score",
        _approx(exception_score, 0.0001),
        str(exception_score),
    )
    ...
    _check(
        "deadline case emits [END]",
        any(line.startswith("[END] ") for line in deadline_lines),
        str(deadline_lines),
    )
    _check(
        "deadline case uses zero-step safe score",
        any(
            line == "[END] success=false steps=0 score=0.0001 rewards=0.0001"
            for line in deadline_lines
        ),
        str(deadline_lines),
    )
    _check(
        "deadline parsed score in (0,1)",
        deadline_end_score is not None and 0.0 < deadline_end_score < 1.0,
        str(deadline_lines),
    )
    _check(
        "deadline run_task returns clamped score",
        _approx(deadline_score, 0.0001),
        str(deadline_score),
    )
