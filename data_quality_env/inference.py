...
LOCAL_IMAGE_NAME: str | None = os.environ.get(
    "LOCAL_IMAGE_NAME"
...
_ISSUE_TYPES: set[str] = {e.value for e in IssueType} if IssueType is not None else set()
_FIX_TYPES: set[str] = {e.value for e in FixType} if FixType is not None else set()
...
def _call_llm(
    ...
    use_json_mode = "api.openai.com" in API_BASE_URL.lower() and any(
        kw in MODEL_NAME.lower() for kw in ("gpt-4", "gpt-3.5", "gpt-4o", "o1", "o3")
    )
    ...
    for attempt in range(retries):
        try:
            ...
            if status_code == 400 and use_json_mode:
                use_json_mode = False
                logger.warning("HTTP 400 with json_mode — retrying without response_format")
                continue
            ...
            if status_code == 400:
                logger.warning("HTTP 400 from API — truncating context for retry")
                if len(messages) > 3:
...
def _obs_to_context(
    ...
    if str(result) != "initial":
        parts.append(
            f"Last action: {result} (reward: {reward_delta:+.3f}, cumulative: {cumulative:.4f})"
        )
    ...
    if uninspected > 0:
        parts.append(
            f"Row coverage: {len(inspected_rows)}/{total_rows} inspected ({uninspected} remaining)"
        )
    else:
    ...
    if step_num == 0:
        ...
        parts.append(
            f"Dataset: {_get(obs, 'total_rows', 0)} rows × {_get(obs, 'total_columns', 0)} columns"
        )
        ...
        with ctx as e:
            ...
            for step_num in range(max_steps):
                ...
                if force_finalize:
                    ...
                    messages.append(
                        {"role": "user", "content": f"Step {step_num}/{max_steps}. Finalizing."}
                    )
                ...
                try:
                    ...
                    logger.warning("Action construction failed: %s — using fallback", exc)
...
def _main_inner() -> int:
    ...
    for task_id in TASKS:
        ...
        try:
            ...
            logger.error("[END] task_id=%s final_reward=0.0001 error=%s", task_id, exc)
            scores[task_id] = _safe_clamp(0.0)
