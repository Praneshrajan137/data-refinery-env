...
DATASETS_DIR: str = str(Path(__file__).resolve().parents[1] / "datasets")
...
PRODUCT_NAMES: dict[str, list[str]] = {
    "Electronics": ["Laptop Pro", "Wireless Mouse", "USB Hub", "Monitor 27in", "Keyboard"],
    "Furniture": ["Office Chair", "Standing Desk", "Bookshelf", "Filing Cabinet", "Desk Lamp"],
    "Clothing": ["Cotton T-Shirt", "Denim Jeans", "Wool Sweater", "Rain Jacket", "Sneakers"],
    "Books": ["Python Guide", "Data Science Handbook", "AI Primer", "ML Textbook", "Stats 101"],
    "Sports": ["Yoga Mat", "Dumbbells 10lb", "Jump Rope", "Resistance Bands", "Water Bottle"],
    "Kitchen": ["Chef Knife", "Cutting Board", "Mixing Bowl Set", "Blender", "Toaster"],
}
...
def rand_date(
    start_year: int = 1960, end_year: int = 2004, rng: random.Random | None = None
) -> str:
    ...
    for i, entry in enumerate(ground_truth):
        ...
        if entry["column"] != "_row":
            assert entry["column"] in schema, (
                f"{prefix}: column '{entry['column']}' not in schema {set(schema.keys())}"
            )

        # Issue type validity
        assert entry["type"] in VALID_ISSUE_TYPES, f"{prefix}: invalid type '{entry['type']}'"
        ...
        assert key not in seen, (
            f"{prefix}: duplicate entry for (row={entry['row']}, column='{entry['column']}')"
        )
        seen.add(key)
...
def generate_task1(
    ...
    ground_truth: list[dict[str, Any]] = [
        # --- FIXABLE ---
        {
            ...
            "description": (
                "Missing @ — 'doeexample' unambiguously splits at 'doe' + 'example' (known domain)"
            ),
        },
        {
            "row": 11,
        ...
        },
        {
            ...
            "expected": "1990-04-30",
            "description": (
                "April 31 invalid — April has exactly 30 days; clamp to maximum valid day"
            ),
        },
        {
            "row": 40,
        ...
        },
        {
            ...
            "description": (
                "Phone contains letters — correct digits unknown without external data"
            ),
        },
        {
            "row": 45,
            ...
            "type": "format_error",
            "original": "ABCDE",
            "description": ("Zip code is all letters — correct code unknown without external data"),
        },
    ]
...
def generate_task2(
    ...
    ground_truth: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "type": "type_mismatch",
            "description": (
                "Phone stored as integer 12345 instead of string — schema requires string type"
            ),
        },
        {
            "row": 110,
            ...
            "type": "type_mismatch",
            "description": (
                "Date '13/05/2023' uses DD/MM/YYYY format instead of required ISO YYYY-MM-DD format"
            ),
        },
    ]
    ...
    rows[100]["email"] = (
        f"{rows[100]['first_name'].lower()}.{rows[100]['last_name'].lower()}@newsletter.info"
    )
...
def generate_task3(
    ...
    product_by_id: dict[int, dict[str, Any]] = {p["product_id"]: p for p in products}
    ...
    p120 = orders[120]
    orders[120]["order_total"] = round(p120["quantity"] * p120["unit_price"] * (1 - 49.99 / 100), 2)
    ...
    p85 = orders[85]
    orders[85]["order_total"] = round(95 * p85["unit_price"] * (1 - p85["discount_pct"] / 100), 2)

    # Row 100: Minimum quantity (1) — valid
    orders[100]["quantity"] = 1
    p100 = orders[100]
    orders[100]["order_total"] = round(1 * p100["unit_price"] * (1 - p100["discount_pct"] / 100), 2)

    # Row 140: Near-zero discount (0.01%) — valid
    orders[140]["discount_pct"] = 0.01
    p140 = orders[140]
    orders[140]["order_total"] = round(p140["quantity"] * p140["unit_price"] * (1 - 0.01 / 100), 2)
    ...
    p215 = orders[215]
    orders[215]["order_total"] = round(p215["quantity"] * p215["unit_price"] * (1 - 50 / 100), 2)
    ...
    orig_total_90 = round(row90["quantity"] * row90["unit_price"] * (1 - orig_discount_90 / 100), 2)
    ...
    _c(
        "task3", orders, 130, "order_total", round(orig_total_130 + 0.009, 3)
    )  # adds tiny rounding error
    ...
    _c("task3", orders, 160, "ship_date", f"{int(y160) - 1}-{m160}-{d160}")
    ...
    _c("task3", orders, 165, "order_total", round(orig_total_165 - 0.01, 2))
    ...
    ground_truth: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "description": (
                f"Negative quantity (-{orig_qty_220}); sign flip — absolute value restores original"
            ),
        },
        ...
        # Business rule: quantity exceeds max
        {
            ...
            "description": (
                "Quantity 500 exceeds max_quantity (100); clamped to business rule maximum"
            ),
        },
        ...
        # STRING TYPE IN NUMERIC FIELD
        {
            ...
            "description": (
                "Quantity stored as string '5' instead of integer; schema requires integer type"
            ),
        },
        # DATE FORMAT ERROR
        {
...
def verify_all() -> bool:
    ...
    expected_files = [
        "task1_customers.json",
        "task1_ground_truth.json",
        "task2_contacts.json",
    ...
    for fname, (exp_total, exp_fixable) in gt_expectations.items():
        ...
        check(f"Issue counts: {fname} ({exp_total} total, {exp_fixable} fixable)", _check_counts)
    ...
    for fname in gt_expectations:
        ...
        def _check_issue_types(f: str = fname) -> None:
            for g in loaded[f]["issues"]:
                _assert(g["type"] in VALID_ISSUE_TYPES, f"invalid type '{g['type']}'")
    ...
    def _check_cross_field_math() -> None:
        ...
        for g in gt["issues"]:
            if g["type"] == "cross_field" and g["column"] == "order_total":
                ...
                _assert(
                    "." in g["expected"],
                    f"row {g['row']}: expected total '{g['expected']}' should have decimal point",
                )
...
def _write_all() -> None:
    ...
    print(
        f"Task 1: {len(ds1['rows'])} rows, {len(gt1)} issues "
    ...
    print(
        f"Task 2: {len(ds2['rows'])} rows, {len(gt2)} issues "
    ...
    _write_dataset(
        "task3_orders.json", ds3["schema"], ds3["rows"], business_rules=ds3["business_rules"]
    )
    ...
    print(
        f"Task 3: {len(ds3['rows'])} orders, {len(products)} products, "
