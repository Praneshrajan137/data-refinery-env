...
def print_diagnostics() -> None:
    ...
    print(separator)
    print(f"  Import root:          {IMPORT_ROOT or 'fallback (openenv-core unavailable)'}")
...
if __name__ == "__main__":
    ...
    if missing_observation_fields:
        print(f"WARNING: Observation is missing expected fields: {missing_observation_fields}")
    else:
        print(f"Observation base fields verified: {sorted(expected_observation_fields)} [OK]")
    ...
    if missing_action_fields:
        ...
        print(f"Action base fields verified: {sorted(expected_action_fields)} [OK]")
    ...
    if missing_state_fields:
        ...
        print(f"State base fields verified: {sorted(expected_state_fields)} [OK]")
    ...
    if create_app is not None:
        ...
        if missing_params:
            ...
            print(f"create_app signature verified: {list(signature.parameters.keys())} [OK]")
    ...
    if Environment is not None:
        ...
        if expected_abstract_methods <= abstract_methods:
            print(
                f"Environment abstract methods verified: {sorted(expected_abstract_methods)} [OK]"
            )
        else:
    ...
    if StepResult is not None:
        ...
        if dataclasses.is_dataclass(StepResult):
            ...
            missing_step_result_fields = expected_step_result_fields - step_result_fields
            if missing_step_result_fields:
                print(f"WARNING: StepResult missing fields: {missing_step_result_fields}")
            else:
                print(f"StepResult fields verified: {sorted(expected_step_result_fields)} [OK]")
        else:
