...
class DataQualityAction(Action):
    ...
    row_indices: Optional[List[int]] = Field(
        None,
        description=(
            f"Row indices to inspect (1–{MAX_INSPECT_ROWS} per call). Each index must be ≥ 0."
        ),
    )
    ...
    row_index: Optional[int] = Field(
        None,
        description="Row index of the suspected or confirmed issue.",
    )
    column_name: Optional[str] = Field(
        None,
        description="Column name of the suspected or confirmed issue.",
    )

    # ── diagnose-specific ─────────────────────────────────────────────────
    issue_type: Optional[IssueType] = Field(
        None,
        description="Classified issue type (see IssueType enum).",
    )

    # ── fix-specific ──────────────────────────────────────────────────────
    fix_type: Optional[FixType] = Field(
        None,
        description="How to fix the issue (see FixType enum).",
    )
    ...
    def _validate_row_indices(cls, v: Optional[List[int]]) -> Optional[List[int]]:
        if v is not None:
            ...
            if len(v) > MAX_INSPECT_ROWS:
                raise ValueError(
                    f"Cannot inspect more than {MAX_INSPECT_ROWS} rows per call (got {len(v)})"
                )
    ...
    def _validate_action_consistency(self) -> "DataQualityAction":
        ...
        if t == "inspect":
            if self.row_indices is None and self.column_names is None:
                raise ValueError(
                    "inspect action requires at least one of 'row_indices' or 'column_names'"
                )
            ...
            if self.fix_type == FixType.CORRECT_VALUE and self.new_value is None:
                raise ValueError(
                    "fix_type 'correct_value' requires 'new_value' (the corrected cell content)"
                )
            if self.fix_type == FixType.DELETE_ROW and self.new_value is not None:
                raise ValueError(
                    "fix_type 'delete_row' must not set 'new_value' (the entire row is removed)"
                )
            ...
            if any(f is not None for f in _optional):
                raise ValueError("finalize action must not set any optional fields")
...
class DataQualityObservation(Observation):
    ...
    visible_rows: Optional[List[Dict[str, Any]]] = Field(
        None,
        description="Rows returned by the last inspect action.",
    )
    ...
    secondary_table_rows: Optional[List[Dict[str, Any]]] = Field(
        None,
        description="Rows from a secondary table (cross-table Task 3).",
    )
    ...
    cumulative_reward: float = Field(
        0.0,
        description="Running total of rewards across the episode.",
    )
    issues_found: int = Field(
        0,
        ge=0,
        description="Number of correctly identified issues so far.",
    )
    ...
    max_steps: int = Field(
        DEFAULT_MAX_STEPS,
        ge=1,
        description="Maximum allowed steps this episode.",
    )
    difficulty_level: Optional[str] = Field(
        None,
        description="Task difficulty tier: easy, medium, or hard.",
    )
...
if __name__ == "__main__":
    ...
    _check("DataQualityAction  ← Action", lambda: assert_(issubclass(DataQualityAction, Action)))
    _check(
        "DataQualityObservation ← Observation",
        lambda: assert_(issubclass(DataQualityObservation, Observation)),
    )
    _check("DataQualityState ← State", lambda: assert_(issubclass(DataQualityState, State)))

    # 2. Base fields inherited
    _check(
        "Action.metadata inherited", lambda: assert_("metadata" in DataQualityAction.model_fields)
    )
    _check(
        "Observation.done inherited", lambda: assert_("done" in DataQualityObservation.model_fields)
    )
    _check(
        "Observation.reward inherited",
        lambda: assert_("reward" in DataQualityObservation.model_fields),
    )
    _check(
        "State.episode_id inherited", lambda: assert_("episode_id" in DataQualityState.model_fields)
    )
    _check(
        "State.step_count inherited", lambda: assert_("step_count" in DataQualityState.model_fields)
    )

    # 3. Factory constructors
    _check(
        "Factory: inspect(row_indices=[0,1])", lambda: DataQualityAction.inspect(row_indices=[0, 1])
    )
    _check(
        "Factory: inspect(column_names=['age'])",
    ...
    _check(
        "Factory: fix(CORRECT_VALUE, new_value='25')",
    ...
    _check("Factory: finalize()", lambda: DataQualityAction.finalize())
    ...
    _must_reject("Reject: inspect with no fields", lambda: DataQualityAction(action_type="inspect"))
    _must_reject(
        "Reject: diagnose missing issue_type",
        lambda: DataQualityAction(action_type="diagnose", row_index=0, column_name="a"),
    )
    _must_reject(
        "Reject: fix missing justification",
    ...
    _must_reject(
        "Reject: correct_value without new_value",
        lambda: DataQualityAction(
            action_type="fix",
            row_index=0,
            column_name="a",
    ...
    _must_reject(
        "Reject: delete_row with new_value",
        lambda: DataQualityAction(
            action_type="fix",
            row_index=0,
            column_name="a",
    ...
    _must_reject(
        "Reject: finalize with extra fields",
        lambda: DataQualityAction(action_type="finalize", row_index=0),
    )
    _must_reject(
        "Reject: row_indices > MAX",
        lambda: DataQualityAction.inspect(row_indices=list(range(MAX_INSPECT_ROWS + 1))),
    )
    _must_reject(
        "Reject: negative row_index",
        lambda: DataQualityAction(
            action_type="diagnose", row_index=-1, column_name="a", issue_type=IssueType.OUTLIER
        ),
    )
    _must_reject(
        "Reject: invalid issue_type string",
        lambda: DataQualityAction(
    ...
    _check("Observation with defaults", lambda: DataQualityObservation())
    _check(
        "Observation with all fields",
    ...
    _check(
        "Observation __repr__ is compact",
        lambda: assert_(
    ...
    _check(
        "Action JSON schema",
        lambda: assert_("action_type" in json.dumps(DataQualityAction.model_json_schema())),
    )
    _check(
        "Observation JSON schema",
        lambda: assert_("action_result" in json.dumps(DataQualityObservation.model_json_schema())),
    )
    _check(
        "State JSON schema",
        lambda: assert_("task_id" in json.dumps(DataQualityState.model_json_schema())),
    )
