...
try:
    ...
    from data_quality_env.server.data_quality_environment import DataQualityEnvironment  # type: ignore
    from data_quality_env.models import DataQualityAction, IssueType  # type: ignore
...
def detect_nulls(rows: list[dict], schema: dict) -> list[dict]:
    ...
    important_cols = {"email", "name", "first_name", "last_name", "phone",
                      "address", "order_total", "unit_price", "quantity"}
...
def detect_cross_field(rows: list[dict], schema: dict, business_rules: dict,
    ...
    for i, row in enumerate(rows):
        ...
        if all(v is not None for v in [qty, price, discount, total]):
            try:
                fqty, fprice, fdisc, ftotal = float(qty), float(price), float(discount), float(total)
        ...
        if odate and sdate and isinstance(odate, str) and isinstance(sdate, str):
            if sdate < odate:
                issues.append({
                    "row": idx, "column": "ship_date",
                    "type": "cross_field", "fix": odate,
                    "confidence": 0.95,
                })
        ...
        if pid in product_by_id and pcat:
            ...
            if real_cat and real_cat != pcat:
                issues.append({
                    "row": idx, "column": "product_category",
                    "type": "cross_field", "fix": real_cat,
                    "confidence": 0.98,
                })
        ...
        if pid is not None and (pid < 0 or pid > 9000):
            issues.append({
                "row": idx, "column": "product_id",
                "type": "referential_integrity", "fix": None,
                "confidence": 1.0,
            })
    ...
    for i, row in enumerate(rows):
        ...
        if discount is not None and max_discount is not None:
            if float(discount) > max_discount:
                issues.append({
                    "row": idx, "column": "discount_pct",
                    "type": "business_rule", "fix": str(max_discount),
                    "confidence": 1.0,
                })
            if float(discount) < 0:
                issues.append({
                    "row": idx, "column": "discount_pct",
                    "type": "business_rule", "fix": "0",
                    "confidence": 1.0,
                })
        ...
        if qty is not None:
            if max_qty and int(qty) > max_qty:
                issues.append({
                    "row": idx, "column": "quantity",
                    "type": "business_rule", "fix": str(max_qty),
                    "confidence": 1.0,
                })
            if min_qty and int(qty) < min_qty:
                issues.append({
                    "row": idx, "column": "quantity",
                    "type": "business_rule", "fix": str(min_qty),
                    "confidence": 1.0,
                })
        ...
        if price is not None and min_price is not None:
            if float(price) < min_price:
                issues.append({
                    "row": idx, "column": "unit_price",
                    "type": "business_rule", "fix": f"{min_price:.2f}",
                    "confidence": 1.0,
                })
...
def detect_duplicates(rows: list[dict], schema: dict) -> list[dict]:
    ...
    key_cols = [c for c in schema if c in ("customer_id", "product_id", "order_date",
...
def run_episode(
    ...
    for start in range(0, total_rows, 10):
        ...
        obs = env.step(DataQualityAction(
            action_type="inspect", row_indices=indices,
        ))
        steps += 1
        if obs.visible_rows:
    ...
    if not obs.done and task_id == "task_3_integrity_auditor":
        obs = env.step(DataQualityAction(
            action_type="inspect", row_indices=[0],
...
def main() -> int:
    ...
    if args.json:
        ...
        for task_id in tasks:
            ...
            std = (sum((s - mean) ** 2 for s in scores) / max(len(scores) - 1, 1)) ** 0.5 if len(scores) > 1 else 0
