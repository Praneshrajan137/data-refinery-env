...
## 2026-09-08 - A capped batch is now held for review, and the guardrail demo proves something again

Two changes, one root cause: **a refusal that reports nothing is indistinguishable from a broken
premise**, and both surfaces had drifted into exactly that state without a gate noticing.

### 1. A volume-capped batch is held, not discarded

**Context**: the previous entry established that 152 verified, ground-truth-correct repairs are
discarded because the batch exceeds `HIGH_VOLUME_CELL_BUDGET = 100`, and that the discard was
**total and silent** — `repair.py` set `accepted_fixes = []` and routed them nowhere, so a user saw
zero repairs *and an empty review queue*.

**Decision**: pre-registered in `eval/preregistration/batch_escalation_visibility.md`. Capture the
batch before emptying it and route it into `receipt.suggested_fixes` via the established
`_suggestion_candidates(...)` shape, at both sites — `run_repair_pipeline` and `verify_and_apply`,
the latter being what the hosted playground calls.

**Result**: on the frozen declared premise, `suggested_fixes` goes **4 → 156**, of which **152**
carry `review_reason: safety_escalation` and name `NO_HIGH_VOLUME_AUTO_APPLY` in `verifier_reason`.
**`result.fixes` stays 0.** 152 is exactly the number `--confirm-escalations` releases, which is the
consistency check that matters.

**K1 — write-set identity — was the criterion this lived or died by, and it is satisfied**: declared
0, oracle 54, every arm unchanged, `anchor_truth` matching on `tp`/`fp`/`fn`/F1 for both corpora. A
visibility fix that altered what is written would not be a visibility fix.

**No new vocabulary and no schema edit** (K2). `evaluate_batch` returns exactly two non-ALLOW
verdicts and both already had exact `ReviewReason` codes — `safety_denied` for the `hard_never`
conflict rule, `safety_escalation` for the soft volume rule — present in the Literal, the humanizer,
the generated TypeScript, and the attestation schema's enum. Inventing `high_volume_escalation`
would have required the four-file hand-copy path whose drift `repair.py:217-219` already records.
The specific cause travels in `verifier_reason`, which is the existing division of labour.

**The cap is untouched** (K3). Refusing to rewrite more than 100 cells unattended is defensible and
is reversible by a documented flag. What was wrong was dropping proven work on the floor while
every other gate in the pipeline holds what it will not auto-apply.

**Not fixed, and named so it is findable**: the identical discard remains at
`dataforge/agent/controller.py:446-452` and `dataforge/stores/repair.py:115-117`. Neither is on the
`dataforge repair` or playground path.

### 2. The guardrail demo proves something again, and the gate that should have caught it was red

**Context**: the playground's untrusted-agent scenario built its premise by accepting MINED
constraints. C4 stopped those conferring write authority, so the demo degraded to uniform refusal —
every proposal held, nothing proven — while its hero copy, its scenario notes, and both endpoint
docstrings continued to promise a proven / held / rejected split.

**How it stayed hidden is the part worth recording.** The integration test was *rewritten* to assert
`would_apply == []` — i.e. to assert the degradation — with a docstring admitting the split was
gone. That left it indistinguishable from `test_verify_fixes_without_schema_holds_everything`.
Meanwhile the Playwright e2e test still asserted `1 proven, 3 blocked`. **Those two committed
assertions are mutually unsatisfiable**, so the playground e2e had been failing since C4: measured
here at 42 passed / 2 failed before this change, 44 passed after. A red gate nobody read.

**Decision**: restore the split with a **DECLARED** premise, not a flag.
`mined_constraints_grant_write_authority` stays unused — the playground must never show a write the
shipped CLI default refuses. `hospital_10rows.schema.yaml` is generated by
`scripts/playground/build_samples.py` and shipped beside the sample; a new form field threads it
through `/api/verify-fixes` to `VerifyAndApplyRequest(schema=...)`, which already had the field and
never populated it. Result: 1 proven, 3 blocked, each for its own distinct reason, and
`independent_verification` honestly becomes `agreed` now that a candidate reaches that gate.

**Why this is honest rather than a dressed-up demo.** The surface verifies a fix *someone else
proposed*; it does not claim DataForge discovers repairs, and new copy says so. And the capability is
real: `docs/trust/fd-repair-yield-mechanism.md` measures a declared premise producing 152 correct
writes at precision 1.0000 on a 1000-row corpus.

**Also corrected**: `authoritative_schema` in the response was `bool(accepted_constraint_ids)`, so
the UI reported "Authoritative schema: 2 accepted constraints" while nothing could be proven. It is
now `bool(declared_schema)`. **No functional dependency is declared** in the sample schema, because
`provider_number -> hospital_name` is FALSE there — PRV001 appears as two different hospitals — and
declaring a refuted dependency to make a demo work would be the premise inflation this project
measures against. Only the discriminating `int`/`float` types confer authority; the `str` columns are
provably inert.

**Scope**: `flights_10rows` and `beers_10rows` ship no declared premise and correctly prove nothing.
Their notes now say so instead of claiming a proven edit. No `beers` work was done beyond correcting
a false sentence, per the dataset-scope rule.

**New gates**: `test_verify_fixes_curated_batch_yields_guardrail_split` now asserts a **non-empty**
`would_apply` — the assertion whose absence let this rot;
`test_verify_fixes_without_a_declared_premise_proves_nothing` pins the contrast;
`test_the_bundled_sample_schema_matches_its_generator` pins the artifact to its generator and to the
CSV header; and `scripts/playground/verify_space_backend.py` now exercises `/api/verify-fixes` at
all, failing if the deployed backend proves nothing.

**Reviewed with**: nobody. K1 was pre-registered and is mechanically checkable.

**Reversal criteria**: if surfacing capped batches is found to mislead users into believing 152
cells were written, revisit the CLI rendering rather than the routing. If the guardrail split ever
requires `mined_constraints_grant_write_authority` to hold, remove the demo instead.
