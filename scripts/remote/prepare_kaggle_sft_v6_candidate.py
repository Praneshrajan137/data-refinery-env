"""Prepare private Kaggle bundles for the DataForge 0.5B SFT-v6 candidate."""
...
DEFAULT_DATASET_DIR = ROOT / "archive" / "training" / "kaggle_sft_v6_handoff"
DEFAULT_KERNEL_DIR = ROOT / "archive" / "training" / "kaggle_sft_v6_kernel"
...
DEFAULT_SFT_CONFIG = ROOT / "archive" / "training" / "configs" / "sft_05b_v6.yaml"
SFT_SCRIPT = ROOT / "scripts" / "remote" / "kaggle_sft_v6_candidate.py"
DATASET_ID = "praneshrajan15/dataforge-sft-v6-handoff"
KERNEL_ID = "praneshrajan15/dataforge-0-5b-sft-v6-candidate"
KERNEL_CODE_FILE = "dataforge-0-5b-sft-v6-candidate.py"
VALID_STAGES = ("smoke", "diagnostic", "candidate")
DEFAULT_STAGE = "smoke"
SOURCE_ROOTS = (
    "dataforge",
    "scripts",
...
def _config_handoff_names(config_path: Path) -> tuple[str, str]:
    ...
    if payload.get("schema_version") != "sft_05b_v6":
        raise ValueError("SFT-v6 handoff requires schema_version=sft_05b_v6.")
    repos = payload.get("repos", {})
    if not isinstance(repos, dict):
        repos = {}
    trajectory_name = str(repos.get("trajectory_filename", "expert_v6_contract_minimal.jsonl"))
    split_manifest_name = str(repos.get("split_manifest_filename", "split_manifest_v4.json"))
    if trajectory_name != "expert_v6_contract_minimal.jsonl":
        raise ValueError(f"Unsupported SFT-v6 trajectory handoff file: {trajectory_name}")
    if split_manifest_name != "split_manifest_v4.json":
        raise ValueError(f"Unsupported SFT-v6 split manifest handoff file: {split_manifest_name}")
...
def build_bundles(
    *,
    ...
    """Build private Kaggle handoff folders for the gated SFT-v6 candidate."""
    ...
    _copy_file(curriculum_report, dataset_dir / "sft_v6_contract_minimal_curriculum_report.json")
    _copy_file(sft_config, dataset_dir / "sft_05b_v6.yaml")
    ...
    candidate_manifest = {
        "schema_version": "dataforge_kaggle_sft_v6_handoff_v1",
        "created_at_utc": datetime.now(UTC).isoformat(),
        "purpose": "0.5B-SFT-v6 private contract-first candidate with strict held-out eval",
    ...
    (dataset_dir / "sft_v6_manifest.json").write_text(
    ...
    (dataset_dir / "dataset-metadata.json").write_text(
        json.dumps(
            {
                "title": "DataForge SFT v6 Handoff",
                "id": DATASET_ID,
    ...
    _write_kernel_script(SFT_SCRIPT, kernel_dir / KERNEL_CODE_FILE, default_stage=default_stage)
    (kernel_dir / "kernel-metadata.json").write_text(
        json.dumps(
            {
                "id": KERNEL_ID,
                "title": "DataForge 0.5B SFT v6 Candidate",
