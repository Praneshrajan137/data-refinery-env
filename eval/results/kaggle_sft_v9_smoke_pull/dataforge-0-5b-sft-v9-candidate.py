...
DEFAULT_SFT_VERSION = os.environ.get("DATAFORGE_SFT_VERSION", "v9")
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(f"Kaggle GPU runtime is required for {input_spec['candidate_label']} candidate.")
    capability = torch.cuda.get_device_capability(0)
    if capability[0] < 7:
        raise RuntimeError(
            f"{input_spec['candidate_label']} candidate requires a T4-or-newer Kaggle GPU for the installed PyTorch wheel; "
